...
int main(){
	...
	*/
	...
	int mayor=0, menor=9999999999, amayor=0, amenor=0;
	for(i=0;i<n;i++){
		if(media[i]>mayor){
			mayor = media[i];
			amayor=i;
	...
	printf("\n\n *+++++++++**+++++++++++++++++++++*++++++++++*");
	printf("\n\t EL alumno con mejor promedio es:	%s \n\t Con nota promedio:	%.2f", alumnos[amayor].nombre, media[amayor]);
		printf("\n\n *+++++++++**+++++++++++++++++++++*++++++++++*");
	printf("\n\t EL alumno con peor promedio es:	%s \n\t Con nota promedio:	%.2f\n\n", alumnos[menor].nombre, media[amenor]);
	system("pause");
	return 0;
}
